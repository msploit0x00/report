...
def get_customer_data(filters):
    ...
    for customer in customers:
        ...
        reasons = frappe.get_all("Customer Validation",
            filters={"parent": customer["name"]},
            fields=["reason"],
        )
        reason_concated = [reason["reason"] for reason in reasons ]
        
        customer["custom_reason_for_susbending"] = ", ".join(reason_concated)
        customer["committees"] = len(committees) 
    return customers    
